  ...
  <div class="address">
    ...
    <div class="addr-list-wrap">
      <ul class="addr-list clearfix">
        <li
          v-for="(receiver,index) in user.receiverInfo"
          :key="index"
          :class="{'check':receiver.isCheck}"
          @click="setCheck(receiver)"
        >
          <dl>
          ...
          <div class="addr-opration addr-del" @click.stop="deleteAddr(index)">
            <i class="el-icon-delete addr-del-btn"></i>
          </div>
          <div
            class="addr-opration addr-set-default"
            v-show="!receiver.isDefault"
            @click.stop="setDefault(receiver)"
          >设为默认地址</div>
    ...
    <div class="next-btn-wrap">
      <el-button-group>
        ...
        <el-button type="danger" @click.native="confirmOrder">订单确认</el-button>
      </el-button-group>
    </div>
...
export default {
  ...
  methods: {
      ...
      User.getUserInfo({ userName }).then(res => {
        if (res.state == "0000") {
          ...
          this.user.receiverInfo.forEach((item, index) => {
            if (item.isDefault) {
              this.$set(this.user.receiverInfo[index], "isCheck", true);
            } else {
              this.$set(this.user.receiverInfo[index], "isCheck", false);
            }
          });
        }
      });
    ...
    confirmOrder() {
        let receiver={};
        this.user.receiverInfo.forEach(item=>{
            if(item.isCheck){
                receiver=item;
            }
        })
      this.$router.push({
          name:'confirmOrder',
          params:{
              checkedList:this.params.checkedList,
              unCheckedList:this.params.unCheckedList,
