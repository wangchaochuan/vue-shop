...
export default {
  ...
  computed: {
    ...
    count() {
      return this.producctList.reduce((sum, item) => {
        return (sum += item.num);
      }, 0);
    },
    userName() {
  ...
  methods: {
    handleChange(product) {
      ...
      mutations.setCount(this.count);
    },
    init() {
      let req = { userName: this.userName },
        result;
      User.getUserInfo(req).then(res => {
        if (res.state == "0000") {
          result = res.data;
          this.producctList = result.shoppingCartList;
          this.user = result;
          this.checkProduct();
        }
      });
  ...
  beforeDestroy() {
    ...
    User.updateShoppingCartList({
      userName,
      shoppingCartList: this.producctList
    }).then(res => {
      if (res.state == "0000") {
        console.log(res);
      }
