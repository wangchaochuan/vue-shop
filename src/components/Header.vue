  ...
  <div class="header">
    <div class="navbar">
      ...
      <div class="navbar-right-container">
        <div class="navbar-menu-container" v-show="!userName">
          ...
          <div class="navbar-cart-container" @click="goShoppingCart">
            <i class="el-icon-shopping-cart-full cart"></i>
          </div>
        </div>
        <div class="navbar-menu-container" v-show="userName">
          ...
          <div class="navbar-cart-container" @click="goShoppingCart">
            <i class="el-icon-shopping-cart-full cart"></i>
            <span v-show="this.count>0" class="listNum">{{this.count}}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      if (userName) {
        User.getShoppingCartList({ userName }).then(res => {
          if (res.state == "0000") {
            let count = res.data.reduce((sum, item) => {
              return (sum += item.num);
            }, 0);
      ...
      User.login({ userName, password })
        .then(resp => {
          ...
          if (resp.result.length > 0) {
            ...
            this.setUser(resp.result[0].userName);
            this.$cookie.set("userName", this.userName, 1);
            let count = resp.result[0].shoppingCartList.reduce((sum, item) => {
              return (sum += item.num);
            }, 0);
            mutations.setCount(count);
          } else {
